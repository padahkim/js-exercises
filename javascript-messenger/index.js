...
function convertToLink(content) {
  if (content.includes("http://")||content.includes("https://")){
      const indexOfWord = content.indexOf("http://")
      const indexOfWord0 = content.indexOf("https://")
      const indexUrlEnd = content.indexOf(" ")
      console.log(content.slice(indexOfWord,indexUrlEnd), content.slice(indexOfWord0,indexUrlEnd))
  }
...
function convertToLink(content) {
  if(content.includes("https://")){
    const words = content.split("https://");
    const text = words[0];
    const url = words[1];
    console.log(url.indexOf("https://"))
    return  `${text} <a href ="https://${url}">https://${url}</a>`;
  }
  else if(content.includes("http://")){
    const words = content.split("http://");
    const text = words[0];
    const url = words[1];
    return `${text} <a href ="http://${url}">http://${url}</a>`;
  }
  // TODO: 인자로 전달되는 텍스트 내용 중에 http:// 혹은 https:// 로 시작하는 텍스트는 a 태그를 이용하여 링크로 변환해주세요.
  // 정규표현식을 사용하지 마세요.
  return content;
...
const chatElement = document.querySelector(".chat__text");
const form = document.querySelector(".form");
const btnElement = form.querySelector(".chat__btn");
...
function createChat(event){
  const chatText = chatElement.value;
  ...
  const chatNew = CHAT_HISTORY[CHAT_HISTORY.length-1];
  const messageElementNew = document.createElement("li");
  messageElementNew.innerHTML = createTextMessage(chatNew);
  messageElementNew.classList.add("message");
  messageListElement.appendChild(messageElementNew);
  chatElement.value="";
  event.preventDefault();
  //createTextMessage(chatText)
}
