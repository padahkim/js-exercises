...
var ajax = new XMLHttpRequest(); //XMLHttpRequest을 실행시켜 리턴값을 ajax변수(ajax객체의 참조변수를 저장)에 저장
...
var newsFeed = JSON.parse(ajax.response); //데이터가 들어온곳 ajax.response를 parse를써서 객체로 변환
...
for (var i = 0; i < newsFeed.length; i++) {
  ...
  li.appendChild(a);
  ul.appendChild(li); //root.appendChild("li")//뭐야이게?? 왜이런코드를 쓴거냐 생각을 하고 코딩을하자,,, 엘리먼트를 만들고 append하는 순서도 중요.
}
